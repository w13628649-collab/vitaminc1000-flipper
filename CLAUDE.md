...
- 界面配色用浅色,沿用旧版 web 端那套:银灰纸底 + 墨色字 + 青铜点缀
  (token 见 `guild/internal/webui/assets/app.css` 的 `:root`)。**用户 2026-09-23 明确要求**
  ("这个黄色背景我也不喜欢,还是以前那种配色好看"),推翻了之前"深暗木底 + 金边 + 羊皮纸字色、
  跟着游戏走"的做法。不要加暗色模式分支,也别再引入 Cinzel 那种游戏风字体
