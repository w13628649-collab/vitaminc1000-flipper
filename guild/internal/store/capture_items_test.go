...
func TestCapturedItems(t *testing.T) {
	...
	// 下面这些都不该出现:黑市/没收敛的地点、品质不在范围里(只问 q1 时)、窗口外、0 件、0 价(解析出错)
	...
	// 同一物品的别的品质单独成行,不并进普通那一行
	sd.put(key("T7_LEATHER", "Martlock", 4, model.SideOffer), fx{9000, 8, T.Add(-30 * time.Minute)})

	cities := []string{"Martlock", "Thetford", "Lymhurst", "Bridgewatch"}
	type row struct {
		id      string
		quality int
		qty     int64
	...
	check := func(qualities []int, want []row) []CapturedItem {
		t.Helper()
		got, err := st.CapturedItems(ctx, cities, qualities, T.Add(-6*time.Hour))
		if err != nil {
			t.Fatal(err)
		}
		if len(got) != len(want) {
			t.Fatalf("品质 %v 应为 %v,得到 %+v", qualities, want, got)
		}
		for i, w := range want {
			g := got[i]
			if g.ItemID != w.id || g.Quality != w.quality || g.Qty != w.qty || g.Orders != w.orders {
				t.Fatalf("品质 %v 第 %d 行应为 %+v,得到 %+v", qualities, i, w, g)
			}
		}
		return got
	}

	got := check([]int{1}, []row{{"T7_LEATHER", 1, 60, 3}, {"T4_STONEBLOCK", 1, 3, 1}, {"T8_CLOTH", 1, 3, 1}})
	if !got[0].LastSeen.Equal(T) {
		t.Fatalf("LastSeen 应为最近那张单的时刻 T,得到 %v", got[0].LastSeen)
	}
	// 扫描列的是五档品质:每个 (物品, 品质) 一行,件数从多到少
	check([]int{1, 2, 3, 4, 5}, []row{
		{"T6_Q2", 2, 999, 1}, {"T7_LEATHER", 1, 60, 3}, {"T7_LEATHER", 4, 8, 1},
		{"T4_STONEBLOCK", 1, 3, 1}, {"T8_CLOTH", 1, 3, 1},
	})
